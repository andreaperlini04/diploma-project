...
def _f(p: dict, key: str, default: str = "?"):
    """Campo del payload, come il ?? di PHP: solo null/assente attivano il
    default. Serve a question_number, dove uno 0 legittimo non va a "?"."""
    value = p.get(key)
    return default if value is None else value


def _named(name_key: str, id_key: str, template: str) -> _Template:
    """Template "nome, con ripiego su #id". Test di verità e non ?? di PHP:
    un nome vuoto dà '#42' invece di virgolette vuote."""
...
def _navigation_clicked(p: dict) -> str:
    """Richiede 'label' o 'target_tag' nel payload: il plugin li rimuoveva
    perché ridondanti con la description calcolata in PHP, ora che la calcola
    il backend senza di essi si degrada a 'User clicked "?"'."""
...
def _page_loaded(p: dict) -> str:
    ...
    Unico template che scende in un oggetto annidato: su un context
    non-dizionario .get() solleverebbe AttributeError fino alla route,
    perdendo l'intero batch invece del solo evento malformato.
    """
    ctx = p.get("context")
