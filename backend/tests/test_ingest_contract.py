...
# Distingue "campo assente" da "campo presente ma null": stesso esito in
# _coerce_ts, ma sono due errori diversi lato client.
OMITTED = object()
...
def test_integer_second_resolution_collides_and_drops_events(client):
    """Il vincolo UNIQUE non comprende il payload: a risoluzione di un secondo
    cinque campioni con alpha diverso sono la stessa riga. Da qui i ms."""
    events = [
...
def test_unknown_source_is_rejected_not_routed_as_moodle(client, db_conn):
    """Un refuso in 'source' finito nel ramo Moodle per esclusione erediterebbe
    session_id e description, sporcando la timeline senza segnalare nulla."""
    body = post(client, [
...
def test_empty_string_description_is_not_recomputed(client, db_conn):
    """Stringa vuota è un valore esplicito, non un'assenza: il backend non la
    rimpiazza. Si rompe sostituendo 'is None' con un controllo di verità."""
    post(client, [
        {"timestamp": 1786544600.0, "source": "moodle",
         "event_type": "navigation_clicked", "payload": {"label": "Avanti"},
         "description": ""},
...
def test_user_id_survives_a_non_dict_context(client, db_conn):
    """Senza il controllo di tipo, .get() su un context stringa solleverebbe
    AttributeError e porterebbe giù l'intero lotto."""
    post(client, [
        {"timestamp": 1786544600.0, "source": "moodle",
...
def test_user_id_is_not_part_of_the_uniqueness_key(client, db_conn):
    ...
    chiave è (session_id, source, event_type, ts). Serve una sessione attiva,
    con session_id nullo il vincolo non scatterebbe mai."""
...
# --------------------------------------------------------------------- #
#
#  Il client EEG non conosce l'utente Moodle: l'unico aggancio è il session_id,
#  condiviso con il plugin.
...
def test_events_are_attributed_within_a_single_batch(client, db_conn):
    """Le entry si assemblano tutte prima dell'INSERT: un campione che precede
    l'evento Moodle nel lotto resterebbe nullo senza il ripescaggio."""
    start_session(client)
...
def test_the_first_user_seen_wins_for_the_whole_session(client, db_conn):
    """Una sessione è di uno studente solo. Un secondo user_id (due account
    sulla stessa macchina) non riscrive l'attribuzione già data: resterebbero
    righe EEG divise fra due studenti senza modo di dire quale sia giusta."""
    start_session(client)
    post(client, [moodle_event(1786544600.0, user_id=7)])
...
def test_attribution_survives_a_backend_restart(db_path, db_conn):
    """I campioni arrivano allo Stop e possono cadere dopo un riavvio: lo stato
    in memoria è perso, l'attribuzione si rilegge dal database."""
...
def test_events_outside_any_session_are_not_attributed(client, db_conn):
    ...
    c'è nessuna sessione a cui appendere lo studente."""
    start_session(client)
    post(client, [moodle_event(1786544600.0, user_id=7)])
